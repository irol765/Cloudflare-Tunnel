...
无需公网 IP，无需开放防火墙端口，隐藏服务器真实 IP，实现安全的加密通信与内网穿透。是作为备用救援线路的绝佳方案。
...
一台 Linux 服务器 (Debian / Ubuntu / CentOS)。

已安装 curl 或 wget。

一个 Cloudflare 账号，并将你的域名托管在 Cloudflare 上。

Cloudflare Tunnel Token (获取方式见下文)。
...
在 "Install and run a connector" 页面，找到下方的命令代码。

复制 --token 后面的那长串字符 (这就是脚本需要的 Token)。
...
回到 Cloudflare Tunnel 页面，点击你的 Tunnel，进入 Public Hostname 标签页。
